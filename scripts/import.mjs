...
const legacyTables = TABLES.filter(t => !ENGAGEMENT_TABLES.includes(t) && !DISCOVERY_TABLES.includes(t));
const coreTables = TABLES.filter(t => !DISCOVERY_TABLES.includes(t));
const migrationCount = Array.isArray(bundle.migrations) ? bundle.migrations.length : -1;
const inputTables = migrationCount === 2 ? legacyTables : migrationCount === 3 ? coreTables : TABLES;
