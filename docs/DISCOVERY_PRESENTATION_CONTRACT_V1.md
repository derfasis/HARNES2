...
| Detail | `GET /api/discovery/:id` | Full situation projection |
...
`discoveryDetail()` is an internal read API and is **not** an authorization boundary on its own.
It does not take an actor. That is stated here rather than hidden behind a second pseudo-auth layer.
...
**Detail** carries evidence, assessments, hypothesis, `why_now`, opening proposals, and review
tasks. Reading it must never be presented as any of the following:
