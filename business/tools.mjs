...
export async function callTool(service, scope, name, args, requestId) {
  ...
  if (name === 'partner_get_context') result = compactPromptContext(contextFor(service, scope.conversationId ?? null));
    ...
    ? service.store.all("SELECT * FROM tasks WHERE conversation_id=? AND kind NOT IN ('opportunity_review','discovery_review') AND status IN ('pending','proposed','running') ORDER BY due_at LIMIT 50", scope.conversationId)
    : service.store.all("SELECT * FROM tasks WHERE partner_id=? AND kind NOT IN ('opportunity_review','discovery_review') AND status IN ('pending','proposed','running') ORDER BY due_at LIMIT 100", service.config.partnerId);
