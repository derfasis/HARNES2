...
const discoveryDecisionQueueQuery = (url, service) => discoveryQueueOptions(url, service);
const discoveryQueueOptions = (url, service) => {
  const options = {};
  for (const [name, values] of url.searchParams) {
    ensure(REASON_STATES_QUERY.has(name) && values.length === 1, 'Неизвестный параметр запроса', 400);
    if (name === 'limit') { ensure(/^[0-9]{1,3}$/.test(values[0]), 'Некорректный limit', 400); options.limit = Number(values[0]); }
    else { ensure(CURSOR_ID.test(values[0]), 'Некорректный cursor', 400); options.cursor = values[0]; }
  }
  return service.discoveryDecisionQueue(options, { kind: 'operator' });
};
  ...
  const server = http.createServer(async (req,res) => {
    ...
    try {
      ...
      if (url.pathname.startsWith('/api/discovery/')) {
        ...
        if (url.pathname === '/api/discovery/decisions') return send(200,discoveryDecisionQueueQuery(url,service));
